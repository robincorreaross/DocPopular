"""
updater.py - Verificador e instalador automático de atualizações do DocPopular.

Fluxo de auto-update no Windows:
 1. Baixa DocPopular.zip para %TEMP%
 2. Extrai para %TEMP%/doc_update/
...
from version import APP_VERSION, DOWNLOAD_URL, DOWNLOAD_ZIP_URL, UPDATE_URL
...
def verificar_atualizacao(
    ...
    def _check() -> None:
        try:
            req = request.Request(
                UPDATE_URL,
                headers={"User-Agent": f"DocPopular/{APP_VERSION}"},
            )
...
def baixar_e_instalar(
    ...
    def _run() -> None:
        ...
        zip_path = tmp_dir / "DocPopular.zip"
        ...
        try:
            ...
            # ── 3. Cria o bat helper ──────────────────────────────────────────
            app_dir = get_app_dir()
            app_exe = app_dir / "DocPopular.exe"
            bat_path = tmp_dir / "update_helper.bat"

            bat_content = f"""@echo off
title DocPopular - Instalando Atualizacao...
